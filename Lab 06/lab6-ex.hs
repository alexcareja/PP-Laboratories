...
count_consecutives [] _ = 0
count_consecutives l a
	| a == head l = 1 + (count_consecutives (tail l) a)
	| otherwise = 0

group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 list
	| length list > 0 = [(take x list)] ++ (group2 (drop x list))
	| otherwise = []
	where
		x = count_consecutives list (head list)
-- Verificare: check4
check4 :: TestPP ()
check4 =
...
isIsogram string = isIsogramHelper (map toLower (filter isLetter string))

isIsogramHelper :: String -> Bool
isIsogramHelper [] = True
isIsogramHelper string = not (elem c (tail string)) && isIsogram (tail string)
	where
		c = head(string)
...
-}
diamond_helper :: Char -> Int -> Int -> [String]
diamond_helper ch outs ins
	| ins == 0 = a ++ (diamond_helper (succ ch) (outs - 1) (2 * ins + 1)) ++ a
	| outs == 0 = [[ch]] ++ (take ins (repeat " ")) ++ [[ch]]
	| otherwise = b ++ (diamond_helper (succ ch) (outs - 1) (2 * ins + 1)) ++ b
	where
		o = (take outs (repeat " "))
		a = o ++ [[ch]] ++ o
		b = o ++ [[ch]] ++ (take ins (repeat " ")) ++ [[ch]] ++ o


diamond :: Char -> [String]
diamond ch = diamond_helper 'A' ((length ['A'..ch]) - 1) 0
