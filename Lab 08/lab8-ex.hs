...
normalizeV param@(V a1 a2 a3) = (V (a1 / n) (a2 / n) (a3 / n))
	where n = lengthV param
...
orthogonalV [x] = True
orthogonalV vectors = isOrthogonalTo (head vectors) (tail vectors) && orthogonalV (tail vectors)

isOrthogonalTo :: Vector -> [Vector] -> Bool
isOrthogonalTo _ [] = True
isOrthogonalTo v vectors 
	| (dotV v (head vectors)) == 0 = isOrthogonalTo (head vectors) (tail vectors)
	| otherwise = False
...
consElem elem (Basic x) = ConsSList [(Basic elem), (Basic x)]
consElem elem (ConsSList xs) = ConsSList ((Basic elem) : xs)

consList :: SList a -> SList a -> SList a
consList x1 y@(Basic x2) = ConsSList [x1, y]
consList x1 (ConsSList x2) = ConsSList (x1 : x2)

headSList :: SList a -> SList a
headSList (ConsSList []) = emptySList
headSList (Basic x) = (Basic x)
headSList (ConsSList x) = head x

tailSList :: SList a -> SList a
tailSList (ConsSList []) = emptySList
tailSList (Basic x) = emptySList
tailSList (ConsSList x) = ConsSList $ tail x

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual (ConsSList []) (ConsSList []) = True
deepEqual (Basic x1) (Basic x2) = (x1 ==  x2)
deepEqual (Basic x1) (ConsSList x2) = False
deepEqual (ConsSList x1) (Basic x2) = False
deepEqual (ConsSList x1) (ConsSList x2)
	| not (length x1 == length x2) = False
	| otherwise = length [x | x <- (zipWith deepEqual x1  x2), not x] == 0
flatten :: SList a -> [a]
flatten (ConsSList []) = []
flatten (Basic x) = [x]
flatten p1@(ConsSList x) = (flatten (headSList p1)) ++ flatten (tailSList p1)
...
data Dice = S1 | S2 | S3 | S4 | S5 | S6 deriving (Eq, Ord, Show)
data YahtzeeRoll = Roll (Dice z1)
{-
...
insertElem (BSTNode v l r) elem
	| v > elem = BSTNode v (insertElem l elem) r
	| otherwise = BSTNode v l (insertElem r elem)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BSTNode v l r) elem
    | v == elem = Just v
    | v > elem = findElem l elem
    | otherwise = findElem r elem

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree BSTNil _ _ = Nothing
subTree n@(BSTNode v l r) a b
	| ((findElem l a) /= Nothing) && ((findElem l b) /= Nothing) = subTree l a b
	| ((findElem r a) /= Nothing) && ((findElem r b) /= Nothing) = subTree r a b
	| ((v == a) && (findElem n b) /= Nothing) || ((v == b) && (findElem n a) /= Nothing) = Just n
	| (((findElem l a) /= Nothing) && ((findElem r b) /= Nothing)) || 
	(((findElem r a) /= Nothing) && ((findElem l b) /= Nothing)) = Just n
	| otherwise = Nothing

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode v l r) = (inorder l) ++ [v] ++ (inorder r)
