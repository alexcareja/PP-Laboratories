...
;; CAREJA ALEXANDRU-CRISTIAN 324CDb
...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg (get-line-segment)) (start (get-start-point seg)) (end (get-end-point seg)))
    (sqrt (+ (sqr (abs (- (car start) (car end)))) (sqr (abs (- (cdr start) (cdr end))))))))
...
(define (distance p1 p2)
  (compute-length (λ () null) (λ (x) p1) (λ (x) p2)))
...
(define (compute-f-with-step f a b step)
  (let iter ((a a) (result '()))
    (if (> a b)
        result
        (iter (+ a step) (append result (list (f a)))))))
...
(define (num-concat x y)
  (let ((+ (λ (x y) (string->number (string-append x y)))) (x (number->string x)) (y (number->string y)))
...
(define (compute-perimeter points)
  (let iter ((points (append points (list (car points)))) (P 0))
    (if (< (length points) 2)
        P
        (iter (cdr points) (+ P (distance (car points) (cadr points)))))))
...
(define (3-sequence-max numbers separator)
  ...
  (let*-values (((seq1 next1) (splitf-at numbers (lambda (x) (not (equal? x separator)))))
                ((seq2 next2) (splitf-at (cdr next1) (lambda (x) (not (equal? x separator)))))
                ((seq3 next3) (splitf-at (cdr next2) (lambda (x) (not (equal? x separator))))))
    (max (sumlist seq1) (sumlist seq2) (sumlist seq3))))
