...
safe(_, []).
safe(X/Y/S, [L/M/N|Others]) :- ((X \== L, Y \== M); S \== N), safe(X/Y/S, Others).
...
% S1.
next_state(misionari, state(est, NME, NCE, NMV, NCV), state(vest, NME2, NCE2, NMV2, NCV2)) :- boat(X, Y), X =< NME, Y =< NCE, NME2 is NME - X,
	NMV2 is NMV + X, NCE2 is NCE - Y, NCV2 is NCV + Y, safeMisionari(NME2, NCE2), safeMisionari(NMV2, NCV2).
next_state(misionari, state(vest, NME, NCE, NMV, NCV), state(est, NME2, NCE2, NMV2, NCV2)) :- boat(X, Y), X =< NMV, Y =< NCV, NME2 is NME + X,
	NMV2 is NMV - X, NCE2 is NCE + Y, NCV2 is NCV - Y, safeMisionari(NME2, NCE2), safeMisionari(NMV2, NCV2).
...
preorder(N, [N|NeighboursFinal]) :- arc(N, Neighbours), preorderHelper(Neighbours, NeighboursFinal).

preorderHelper([],[]).
preorderHelper([N|Rest], Result) :- preorder(N, NNeigh), preorderHelper(Rest, RestNeigh), append(NNeigh, RestNeigh, Result).
