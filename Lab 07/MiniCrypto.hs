...
arithmetic x r = iterate (\x -> x + r) x
geometric :: Num a => a -> a -> [a]
geometric x q = iterate (\x -> x * q) x
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = map (fromIntegral.fst) pipe
  where
    generator = mkStdGen seed
    start = next generator
    pipe = iterate getNext start
    getNext (value, generator) = next generator
...
wordToAlpha x = chr (97 + ((fromIntegral x) `mod` 26))
{-
    5. (1p)
...
tableToFunc lista = fun
  where
    fun x = snd $ head $ (filtered x)
    filtered s = filter (\y -> (fst y) == s) lista
...
genRotTable offset = zip  ['a'..'z'] crypted
  where
    crypted = map (\x -> chr (97 + ((offset + ((ord x) - 97)) `mod` 26)))  ['a'..'z']
...
encryptVigenere original key = zipWith fun original (cycle key)
  where
    fun x y = chr $ 97 + (((ord x) - 97) + ((ord y) - 97)) `mod` 26
{- 9, (Bonus - 2p)
...
xorStrings s1 s2 = map chr $ zipWith xor (fun s1) $ fun s2 
  where
    fun = map ord
...
rollingXor s1 s2 
  |length s1 < length s2 = xorStrings (cycle s1) s2
  |otherwise = xorStrings s1 (cycle s2)
