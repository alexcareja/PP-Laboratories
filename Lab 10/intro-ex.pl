...
myConcat([X|Tail], List1, [X|Rest]) :- myConcat(Tail, List1, Rest).
...
add_odds([], []).
add_odds([X|Y|Tail], R) :- A is mod(X, 2), B is mod(Y, 2), X =:= 1, Y =:= 1, Z is X + Y, append([Z], Tail, R).
add_odds([X|Tail], R) :- append([X], R2, R), add_odds(Tail, R2).
...
myReverse([X],[X]).
myReverse([X|Tail], Y) :- myReverse(Tail, Z), myConcat(Z, [X], Y).
...
myReverseAcc([X|Tail], Acc, Y) :- myReverseAcc(Tail, [X|Acc], Y).
...
extract([], _, _, []).
extract([X|_], 0, 0, [X]).
extract([X|Tail], 0, End, [X|Range]) :- NewEnd is End - 1,extract(Tail, 0, NewEnd, Range).
extract([_|Tail], Start, End, Range) :- NewStart is Start - 1, NewEnd is End - 1, extract(Tail, NewStart, NewEnd, Range).
...
factorial(0, 0).
factorial(1, 1).
factorial(N, Fact) :- NewN is N - 1, factorial(NewN, M), Fact is N * M.
...
sublista([], []).
sublista([X|XTail], [X|YTail]) :- sublista(XTail, YTail).
sublista([X|XTail], SubL) :- sublista(XTail, SubL).
...
descendantOf(X,Y):- arc(Y, Z), descendantOf(X, Z).
...
descendants(Nod, 0) :- \+ arc(Nod, _).
%%descendants(Nod, 1) :- arc(Nod, X), \+ (arc(Nod, Y), X \= Y), descendants(X, 0).
descendants(Nod, 2) :- arc(Nod, X), arc(Nod, Y), X \= Y, descendants(X, 0), descendants(Y, 0).
descendants(Nod, N) :- arc(Nod, X), \+ (arc(Nod, Y), X \= Y), descendants(X, XD), N is XD + 1.
descendants(Nod, N) :- arc(Nod, X), arc(Nod, Y), X \= Y, descendants(X, XD), descendants(Y, YD), N is XD + YD + 2.
