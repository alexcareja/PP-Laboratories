...
class (Ord a) => PQueue pq a where
    ...
    fromList [] = empty
    fromList toInsert = insert (head toInsert) $ fromList $ tail toInsert

    toList :: pq a -> [(Prio, a)]
    toList prioQ
    	| isEmpty prioQ = []
    	| t == [] = []
    	| otherwise = [head t] ++ (toList $ pop prioQ)
    		where t = myFromMaybe $ top prioQ

    size :: pq a -> Int
    size prioQ = length $ toList prioQ -- Pentru Ex 6
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert elem (LPQ q)
    	| q == [] = LPQ [elem]
    	| fst elem < fst (head q) = LPQ $ (head q) : (toList $ insert elem $ LPQ $ tail q)
    	| otherwise = LPQ $ elem : q
...
merge (Empty 0) t = t
merge t (Empty 0) = t
merge t1@(Node _ (p1, elem1) l1 r1) t2@(Node _ (p2, elem2) l2 r2)
  | p1 >= p2 && (rank l1 >= rank t3) = (Node (1 + (rank t3)) (p1, elem1) l1 t3)
  | p2 > p1 && (rank l2 >= rank t4) = (Node (1 + (rank t4)) (p2, elem2) l2 t4)
  | p1 >= p2 && (rank l1 < rank t3) = (Node (1 + rank l1) (p1, elem1) t3 l1)
  | otherwise = (Node (1 + rank l2) (p2, elem2) t4 l2)
    where 
      t3 = merge r1 t2
      t4 = merge r2 t1
...
inorder (Empty 0) = []
inorder (Node _ el l r) = (inorder l) ++ [el] ++ (inorder r)
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert elem = merge $ Node 1 elem (Empty 0) (Empty 0)

    top q
      | isEmpty q = Nothing
      | otherwise = Just $ nodeVal q

    pop q = merge (left q) (right q)
...
instance MyFunctor ListPQ where
    fmap' f (LPQ q) = fromList $ map f q

instance MyFunctor LeftistPQ where
    fmap' f t = fromList $ map f $ inorder t
